...
export const CartCard = ({ product }) => {
    const fail = () => toast.error("Ooops! Error on our side")
    const format = Intl.NumberFormat('en-US')
    const { name, price, image, _id } = product
    const { removeFromCart, changeTotalList} = useCart()
    const [quantity, setQuantity] = useState(1)
    let subtotal_price = quantity * price
    const handleInputChange = (e) => {
        setQuantity(e.target.value < 1? quantity:e.target.value);
        try {
            changeTotalList(subtotal_price, _id)
        } catch {
            fail()
        }
    }
        ...
        <Toaster />
          ...
          <div className="p-2 flex justify-start flex-wrap align-middle">
              <div className= "flex justify-start align-middle h-full">
                    ...
                    <input type='number' value={quantity} onChange={(e) => handleInputChange()}></input>
              </div>
